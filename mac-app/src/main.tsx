...
function formatShortcut(settings: Pick<AppSettings, 'shortcutModifier' | 'shortcutKey'>) {
  return `${formatModifier(settings.shortcutModifier)}+${settings.shortcutKey}+${settings.shortcutKey}`;
}

function formatModifier(modifier: AppSettings['shortcutModifier']) {
        ...
        <section className="pane inputPane">
          ...
          {!input && (
            <div className="emptyState">
              <p>
                {t.quickTipPrefix} <kbd>{formatModifier(settings.shortcutModifier)}</kbd> + <kbd>{settings.shortcutKey}</kbd> +{' '}
